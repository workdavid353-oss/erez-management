...
# ── 8. Backup Script ───────────────────────────────────────────────────────────
Title "Backup Setup"
Copy-Item "$ProjectDir\C:\actions-runner\backup.ps1" "$RunnerDir\backup.ps1" -ErrorAction SilentlyContinue

# Write backup.ps1 directly
$backupScript = @'
$ErrorActionPreference = 'Stop'
...
'@
[System.IO.File]::WriteAllText("$RunnerDir\backup.ps1", $backupScript)
...
if ($setupBackup -eq 'y') {
    Write-Host ""
    Write-Host "  IMPORTANT: Copy rclone.conf from the old machine to:" -ForegroundColor Yellow
    Write-Host "  C:\Users\$env:USERNAME\AppData\Roaming\rclone\rclone.conf" -ForegroundColor White
    Write-Host "  Then run: C:\actions-runner\rclone.exe config (if first time)" -ForegroundColor White
    ...
    Info "Skipped backup setup — run C:\actions-runner\rclone.exe config manually then re-run setup"
}
